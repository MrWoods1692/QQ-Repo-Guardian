...
impl IntoIterator for MessageChain {
    ...
    type IntoIter = std::iter::FilterMap<
        std::vec::IntoIter<MessageElem>,
        fn(MessageElem) -> Option<RQElem>,
    >;

    fn into_iter(self) -> Self::IntoIter {
        self.0
            .into_iter()
            .filter_map(message_elem_into_rq_elem)
    }
}

fn message_elem_into_rq_elem(element: MessageElem) -> Option<RQElem> {
    if matches!(element, MessageElem::SrcMsg(_) | MessageElem::AnonGroupMsg(_)) {
        None
    } else {
        Some(RQElem::from(element))
    }
}
